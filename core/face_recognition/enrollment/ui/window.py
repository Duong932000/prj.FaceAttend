...
# load config
enroll_camera_cfg, _, _, _, \
enroll_pose_cfg, enroll_pipeline_cfg, _, \
enroll_color_detection_cfg, enroll_color_overlay_cfg  = load_enrollment_config()

class FaceCollectionOptions(customtkinter.CTkScrollableFrame):
    def __init__(self, master, height=300, command=None, **kwargs):
        super().__init__(master, height=height, orientation="vertical", **kwargs)
...
class MainWindow(customtkinter.CTk):
    ...
    def cameraUpdateFrame(self):
        ...
        if frame is not None:
            ...
            if panel_width > 10 and panel_height > 10:
                ...
                # face detection drawling
                if self.app_mode in ["detection", "enrollment"] and result is not None and result["face_detected"]:
                    face = result["face"]
                    ...
                    # enrollment validation only
                    if self.app_mode == "enrollment":
                    ...
                    # confidence text
                    confidence_text = (f"Conf: {score:.2f}")
                    cv2.putText(frame, confidence_text, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, box_color, 2)

                    # pose text
                    pose_text = (f"Yaw:{yaw:.1f} " f"Pitch:{pitch:.1f}")
                    cv2.putText(frame, pose_text, (x1, y2 + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, box_color, 2)

                if self.app_mode == "enrollment":
                    self.drawEnrollmentGuide(frame)

                    target_pose_text = (f"Target Pose: "f"{self.face_processor.target_pose.upper()}")
                    cv2.putText(frame, target_pose_text, (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                                tuple(enroll_color_detection_cfg["bbox"]["green"]), 2)
        ...
        self.after(10,self.cameraUpdateFrame)
    ...
    def faceDetectionProcessor(self):
        ...
        self.after(100, self.faceDetectionProcessor)
